package com.example.spock.util;
...
public class Error {
  ...
  static Builder error(String code) {
    return new Builder().withCode(code);
  }

  private static Builder like(Error other) {
    return new Builder()
  ...
  public String toString() {
    return String
        .format("Error{code='%s', message='%s', details='%s', path='%s', userMessage='%s'}", code,
            message, details, path, userMessage);
  }
  ...
  public static final class Builder {
    ...
    public Builder fromException(Throwable exception, boolean withStackTrace) {
      ExceptionMappingMode mode = withStackTrace ?
          ExceptionMappingMode.INTERNAL_DETAILS_WITH_STACKTRACE
          : ExceptionMappingMode.NO_INTERNAL_DETAILS;
    ...
    public Builder addPrefixToPath(String prefix) {
      if (this.path != null) {
        this.path = prefix + Error.PATH_SEPARATOR + this.path;
      } else {
        this.path = prefix;
      }
      return this;
